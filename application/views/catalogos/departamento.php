      ...
      <header id="topbar" class="alt">
        ...
        <a class="btn btn-info pull-right" href="<?= base_url() . "principal/nuevoDepartamento" ?>">Agregar Nuevo <span class="glyphicon glyphicon-book"></span></a>
			              ...
			              <div class="panel panel-visible" id="spy5">
			                <div class="panel-heading">
			                  <div class="panel-title hidden-xs">
			                    <span class="glyphicon glyphicon-tasks"></span>Gestion de Departamentos</div>
			                </div>
			                <div class="panel-body pn">
                              ...
                              <td>
                                 <?php foreach($areas as $area): ?>
                                          <?php if( ($area->idArea) == ($item->Area_idArea ) ) : ?>
                                            <?= $area->Nombre?>
                                          <?php endif; ?>
                                <?php endforeach; ?>
                              </td>
			                    		<td><a class="btn btn-info btn-md" href="<?php echo base_url() . "principal/editarDepartamento/" . $item->idDepartamento?>"><span class="glyphicon glyphicon-pencil"></span></a></td>
			                    		<td><button class="eliminar btn btn-danger btn-md" data-val="<?=$item->idDepartamento ?>"><span class="glyphicon glyphicon-trash"></span></button></td>
			                    	</tr>
			                    	<?php endforeach; ?>
			                    	<?php endif; ?>
			                    </tbody>
...
<!-- BEGIN: PAGE SCRIPTS -->
  ...
  jQuery(document).ready(function() {
    ...
    Core.init();
    var columnas = [0, 1 , 2];
    InitTable("tabla","CATALOGO DE DEPTOS" , "landscape" ,columnas);
	    	...
	    	bootbox.confirm("<center>¿Esta seguro que desea eliminar el registro? <br></center>", function(confirmed) {
                        if(confirmed==true)
                        {
                        	id= OEliminar.data("val");
		                     datastr = "idDepartamento=" + id ;
		                            $.ajax({
		                                type: "POST", 
		                                async: false, 
		                                url: '<?php echo base_url(); ?>principal/eliminaDepartamento',
		                                data: datastr, 
		                                dataType: "text",
		                                cache:false,
