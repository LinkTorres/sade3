      ...
      <header id="topbar" class="alt">
        <div class="topbar-left">
          ...
          <a class="btn btn-info pull-right" href="<?= base_url() . "principal/nuevaUbicacion" ?>">Agregar Nueva <span class="glyphicon glyphicon-book"></span></a>
      </header>
      <!-- End: Topbar -->
...
.panel-controls{display: none;}</style>
			              ...
			              <div class="panel panel-visible" id="spy5">
			                <div class="panel-heading">
			                  <div class="panel-title hidden-xs">
			                    <span class="glyphicon glyphicon-tasks"></span>Gestion de Ubicaciones</div>
			                </div>
			                  ...
			                  <table class="text-center table table-striped table-hover display" id="tabla" cellspacing="0" width="100%">
			                    ...
			                    <tbody >
			                    	...
			                    	<tr>
			                    		...
			                    		<td><?= $item->Nombre ?></td>
			                    		<td><a class="btn btn-info btn-md" href="<?php echo base_url() . "principal/editarUbicacion/" . $item->idUbicacion?>"><span class="glyphicon glyphicon-pencil"></span></a></td>
			                    		<td><button class="eliminar btn btn-danger btn-md" data-val="<?=$item->idUbicacion ?>"><span class="glyphicon glyphicon-trash"></span></button></td>
			                    	</tr>
			                    	<?php endforeach; ?>
			                    	<?php endif; ?>
			                    </tbody>
 ...
 <!-- DATATABLES -->
  ...
  jQuery(document).ready(function() {
    ...
    Core.init();
    var columnas = [0, 1];
    InitTable("tabla","CATALOGO DE UBICACIONES" , "landscape" ,columnas);
	    	...
	    	bootbox.confirm("<center>¿Esta seguro que desea eliminar el registro? <br></center>", function(confirmed) {
                        if(confirmed==true)
                        {
                        	id= OEliminar.data("val");
		                     datastr = "idUbicacion=" + id ;
		                            $.ajax({
		                                type: "POST", 
		                                async: false, 
		                                url: '<?php echo base_url(); ?>principal/eliminaUbicacion',
		                                data: datastr, 
		                                dataType: "text",
		                                cache:false,
