      <header id="topbar" class="alt">
        <div class="topbar-left">
          <ol class="breadcrumb">
            ...
            <li class="crumb-icon">
              <a href="<?= base_url() . "principal/areas" ?>">
			              ...
			              <div class="panel panel-visible" id="spy5">
			                <div class="panel-heading">
			                  <div class="panel-title hidden-xs">
			                    <span class="glyphicon glyphicon-tasks"></span>Editar Área</div>
			                </div>
                      
			                <div class="panel-body pn">
                        <?php if(!empty($area)): ?>
                        <h5 class="text-center">Actualiza la información</h5>
			                  <form id="actualiza" data-parsley-validate action="<?php echo base_url();?>principal/SEditArea" method="post" role="form">
                            <div class="row">
                              <label class="col-md-2 col-md-offset-1 control-label" for="">ID</label>
                                <div class="col-xs-6">
                                    <div class="form-group">
                                      <input type="text" name="idArea" value="<?=$area->idArea ?>" class="form-control input-sm" readonly required>
                                    </div>
                                </div>
                            ...
                            <div class="row">
                              <label class="col-md-2 col-md-offset-1 control-label" for="">Dirección</label>
                                <div class="col-xs-6 ">
                                    <div class="form-group">
                                      <select class="form-control" name="Direccion_idDireccion">
                                          <?php foreach($direcciones as $item): ?>

                                          <?php if( ($item->idDireccion) == ($area->Direccion_idDireccion ) ) : ?>
                                          <option selected="selected" value="<?= $item->idDireccion ?>"><?= $item->Nombre ?></option>
                                          <?php else: ?>
                                          <option value="<?= $item->idDireccion ?>"><?= $item->Nombre ?></option>
                                          <?php endif; ?>         
                             ...
                             <div class="row">
                              <label class="col-md-2 col-md-offset-1 control-label" for="">Nombre</label>
                                <div class="col-xs-6">
                                    <div class="form-group">
                                      <input type="text" name="nombre" value="<?=$area->Nombre ?>" class="form-control input-sm" placeholder="Nombre" required>
                                    </div>
                                </div>
                            </div>
