      ...
      <header id="topbar" class="alt">
        ...
        <a class="btn btn-info pull-right" href="<?= base_url() . "principal/nuevaArea" ?>">Agregar Nueva <span class="glyphicon glyphicon-book"></span></a>
			              ...
			              <div class="panel panel-visible" id="spy5">
			                <div class="panel-heading">
			                  <div class="panel-title hidden-xs">
			                    <span class="glyphicon glyphicon-tasks"></span>Gestion de Áreas</div>
			                </div>
			                <div class="panel-body pn">
                              ...
                              <td>
                                <?php foreach($direcciones as $direccion): ?>
                                          <?php if( ($direccion->idDireccion) == ($item->Direccion_idDireccion ) ) : ?>
                                            <?= $direccion->Nombre?>
                                          <?php endif; ?>
                                <?php endforeach; ?>
                              </td>
			                    		<td><a class="btn btn-info btn-md" href="<?php echo base_url() . "principal/editarArea/" . $item->idArea?>"><span class="glyphicon glyphicon-pencil"></span></a></td>
			                    		<td><button class="eliminar btn btn-danger btn-md" data-val="<?=$item->idArea ?>"><span class="glyphicon glyphicon-trash"></span></button></td>
			                    	</tr>
			                    	<?php endforeach; ?>
			                    	<?php endif; ?>
			                    </tbody>
...
<!-- BEGIN: PAGE SCRIPTS -->
  ...
  jQuery(document).ready(function() {
    ...
    Core.init();
    var columnas = [0, 1 , 2];
    InitTable("tabla","CATALOGO DE AREAS" , "landscape" ,columnas);
                        	...
                        	id= OEliminar.data("val");
		                     datastr = "idArea=" + id ;
		                            $.ajax({
		                                type: "POST", 
		                                async: false, 
		                                url: '<?php echo base_url(); ?>principal/eliminaArea',
		                                data: datastr, 
		                                dataType: "text",
		                                cache:false,
